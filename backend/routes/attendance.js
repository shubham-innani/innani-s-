...
const { format, getDay, endOfMonth, eachDayOfInterval, parseISO } = require('date-fns');
...
router.get('/daily', protect, async (req, res) => {
  try {
    ...
    // Fetch ONLY active workers for the daily attendance marking view
    const workers = await Worker.find({ isActive: true }).sort({ createdAt: 1 });

    const attendanceMap = {};
    attendancesForDate.forEach(a => {
      attendanceMap[a.workerId.toString()] = a;
    });
    ...
    if (adminUser.lastChangeDate === today) {
      if (adminUser.dailyChangeCount >= 10) {
        return res.status(429).json({
          message: 'You have reached today\'s attendance change limit of 10.'
        });
      }
    } else {
...
router.get('/monthly', protect, async (req, res) => {
  try {
    ...
    if (
      ...
      workerId !== req.user.workerId.toString()
    ) {
      return res.status(403).json({
    ...
    // Use date-only values for monthly calculations.
    // Attendance dates are stored as YYYY-MM-DD strings,
    // so avoid timezone conversion at the month boundaries.
    ...
    const workers = workerId
      ? await Worker.find({ _id: workerId })
      : await Worker.find({
        $or: [
          { isActive: true },
          {
            _id: {
              $in: Object.keys(dataByWorker)
            }
          }
        ]
      });
    ...
    const response = workers.map(w => {
      ...
      days.forEach(day => {
        ...
        const isBeforeJoining =
          day < joined &&
          format(day, 'yyyy-MM-dd') !== format(joined, 'yyyy-MM-dd');

        const hasRecord =
          dataByWorker[wid] &&
          dataByWorker[wid][dateStr];
      ...
      const attendancePercentage =
        workingDays > 0
          ? ((totalEquivalent / workingDays) * 100).toFixed(2)
          : '0.00';
